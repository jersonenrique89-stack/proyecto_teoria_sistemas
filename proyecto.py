...
def procesar_imagen(ruta_imagen, filtros=None):
    print("***** EJECUTANDO PROYECTO.PY *****")
    print("Ruta recibida:", ruta_imagen)
    print("Filtros seleccionados:", filtros if filtros else "todos")

    # Si no se especifican filtros, se aplican todos por defecto
    if filtros is None:
        filtros = ["gris", "suavizada", "gaussiana", "pasa_altas", "enfocada", "bordes", "fourier"]
    ...
    # Solo se aplican los filtros seleccionados por el usuario

    imagen_suavizada = None
    imagen_gaussiana = None
    imagen_pasa_altas = None
    imagen_enfocada = None
    imagen_bordes = None
    espectro = None

    # Aplicar filtro de suavizado
    if "suavizada" in filtros:
    ...
    if "gaussiana" in filtros or "pasa_altas" in filtros:
    ...
    # Solo se calcula si el usuario seleccionó "fourier"
    if "fourier" in filtros:
    ...
    # Solo se convierten los filtros que fueron seleccionados por el usuario
    ...
    # --- CONSTRUIR RESPUESTA ---
    # Solo se incluyen los filtros seleccionados por el usuario
    respuesta = {
        "mensaje": "Procesamiento completado",
        "original": imagen_a_base64(imagen),
    ...
    # Escala de grises
    if "gris" in filtros:
        respuesta["gris"] = imagen_a_base64(imagen_gris)
        respuesta["gris_descripcion"] = (
    ...
    # Suavizado promedio
    if "suavizada" in filtros and imagen_suavizada is not None:
        respuesta["suavizada"] = imagen_a_base64(imagen_suavizada)
        respuesta["suavizada_descripcion"] = (
    ...
    # Gaussiano
    if "gaussiana" in filtros and imagen_gaussiana is not None:
        respuesta["gaussiana"] = imagen_a_base64(imagen_gaussiana)
        respuesta["gaussiana_descripcion"] = (
    ...
    # Pasa altas
    if "pasa_altas" in filtros and imagen_pasa_altas is not None:
        respuesta["pasa_altas"] = imagen_a_base64(imagen_pasa_altas)
        respuesta["pasa_altas_descripcion"] = (
    ...
    # Enfoque
    if "enfocada" in filtros and imagen_enfocada is not None:
        respuesta["enfocada"] = imagen_a_base64(imagen_enfocada)
        respuesta["enfocada_descripcion"] = (
    ...
    # Bordes
    if "bordes" in filtros and imagen_bordes is not None:
        respuesta["bordes"] = imagen_a_base64(imagen_bordes)
        respuesta["bordes_descripcion"] = (
    ...
    # Fourier
    if "fourier" in filtros and espectro is not None:
        respuesta["fourier"] = imagen_a_base64(espectro)
        respuesta["fourier_descripcion"] = (
